...
void mcu_init(void) {
  ...
  for let's say a weather balloon or spacecraft where manual reset by human interaction 
  is not possible. In some mcus the WDT is on by default. To check if the WDT on mcu (SAMD21G17D) 
  is on by default, we have to refer the datasheet of the mcu.
  */
